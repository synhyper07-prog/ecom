...
class ApiController extends BaseController{ 
    ...
    */
    public function calculateShippingCharge(Request $request){
        $user  = Auth::user();
        $validator = Validator::make($request->all(), [
            'vendor_order' => 'required'
        ]);

        $tot_delivery_charge_arr = array();
        $vendor_wise_delivery_charge = array();
        foreach ($request->vendor_order as $key => $value) {
        	$vendor_detail = User::where('id', $value['user_id'])->first('pincode');
            $data  = str_replace("'", '"', $value['items']);
        	$items_detail = json_decode($data);
        	
        	$item_array  = array();
        	foreach ($items_detail->item_id as $key => $item_value) {
        		$check_free_delivery = Product::where('id', $item_value->id)->first('free_delivery');
        		if($check_free_delivery->free_delivery == 0){
        		   array_push($item_array, $item_value->id);
        		}
        	}
            ...
            $get_vendor_ship_charge  = $this->get_vendor_ship_charge($vendor_detail->pincode, $value['shipping_pincode'], $height, $width, $length, $weight);

            $vendor_delivery = array('user_id'=>$value['user_id'], 'delivery_charge'=>$get_vendor_ship_charge);
            array_push($vendor_wise_delivery_charge, $vendor_delivery);
            array_push($tot_delivery_charge_arr, $get_vendor_ship_charge);
        }

        $success['tot_delivery_charge']         = array_sum($tot_delivery_charge_arr);
        $success['vendor_wise_delivery_charge'] = $vendor_wise_delivery_charge;
        if ($validator->fails()) {
          return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
        }
        return $this->sendResponse($success, 'success');     
