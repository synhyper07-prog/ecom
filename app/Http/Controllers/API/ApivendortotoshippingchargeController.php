    ...
    */
    public function calculateShippingCharge(Request $request){
        ...
        foreach ($vendor_wise_cart as $key => $cart_value) {
            ...
            $get_vendor_ship_charge  = $this->get_vendor_ship_charge($vendor_detail->pincode, $request->shipping_zip, $height, $width, $length, $weight, $mrp);
            $single = count($item_array);
            DB::table('productcarts')->where('is_active', '1')->where('order_id', '0')->where('vendor_id', $cart_value->vendor_id)->where('product_id', $item_value->id)->update(['shipping_cost' =>$get_vendor_ship_charge/$single]);
    ...
    */
    public function checkout(Request $request){
        ...
        foreach ($vendor_wise_cart as $key => $cart_value) {
            ...
            $mrp    = Product::whereIn('id', $item_array)->sum('price');
            $get_vendor_ship_charge  = $this->get_vendor_ship_charge($vendor_detail->pincode, $request->shipping_zip, $height, $width, $length, $weight, $mrp);
            $this->create_suborder($request, $get_vendor_ship_charge, $cart_value->vendor_id, $order_detail->id, $payment_method, $status);
        }
