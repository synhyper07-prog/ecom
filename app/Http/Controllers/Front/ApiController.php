...
class ApiController extends BaseController{ 
    ...
    */
    public function addCart(Request $request){
        ...
        $item_info['vendor_id']    = $prod->user_id;
    ...
    */
    public function calculateShippingCharge(Request $request){
        ...
        $tot_delivery_charge_arr = array();
        $vendor_wise_cart = $this->bifercate_cart_vendor_wise();
        foreach ($vendor_wise_cart as $key => $cart_value) {
            $vendor_detail = User::where('id', $cart_value->vendor_id)->first('pincode');
            if(!empty($vendor_detail)){
              $pincode = $vendor_detail->pincode;
            }
            else{
              $pincode = '226023'; 
            }    
            $item_array = array();
            foreach ($cart_value->cart as $key => $item_value) {
            ...
            $mrp    = Product::whereIn('id', $item_array)->sum('price');
            $get_vendor_ship_charge  = $this->get_vendor_ship_charge($pincode, $request->shipping_pincode, $height, $width, $length, $weight, $mrp);
            $single = count($item_array);
            DB::table('productcarts')->where('is_active', '1')->where('order_id', '0')->where('vendor_id', $cart_value->vendor_id)->where('user_id', Auth::user()->id)->update(['shipping_cost' =>$get_vendor_ship_charge/$single]);
            array_push($tot_delivery_charge_arr, $get_vendor_ship_charge);   
        }
        $success['tot_delivery_charge'] = array_sum($tot_delivery_charge_arr);
        if ($validator->fails()) {
           return response()->json(array('errors' => $validator->getMessageBag()->toArray()));
        }
        return $this->sendResponse($success, 'success');     
    ...
    */
    public function checkout(Request $request){
        ...
        foreach ($vendor_wise_cart as $key => $cart_value) {
        	  $vendor_detail = User::where('id', $cart_value->vendor_id)->first('pincode');
            $this->create_suborder($request, $cart_value->tot_shipping_cost, $cart_value->vendor_id, $order_detail->id, $payment_method, $payment_status);
        }
        $order_track = array();
    ...
    */
    ...
    /**
    * Get order product detail  api
    *
    * @return \Illuminate\Http\Response
    */
    public function orderProductDetail(Request $request){
        $user_order_list = DB::table('orders')->where('id', $request->order_id)->get('cart_itmes');
        $success['order_detail']      =  DB::table('orders')->where('id', $request->order_id)->get(['id', 'user_id', 'method', 'shipping', 'pickup_location',  'totalQty', 'pay_amount', 'txnid', 'charge_id', 'order_number', 'payment_status', 'customer_email', 'customer_name', 'customer_country', 'customer_phone', 'customer_address', 'customer_city', 'customer_zip', 'shipping_name', 'shipping_country', 'shipping_email', 'shipping_phone', 'shipping_address', 'shipping_city', 'shipping_zip', 'order_note', 'coupon_code', 'coupon_discount', 'status', 'created_at', 'updated_at', 'affilate_user', 'affilate_charge', 'currency_sign', 'currency_sign', 'shipping_cost', 'packing_cost', 'tax', 'dp', 'pay_id', 'vendor_shipping_id', 'vendor_packing_id']);
        $success['order_item_detail'] = Productcart::join('products', 'products.id', '=', 'productcarts.product_id')->where('productcarts.is_active','=', '0')->where('productcarts.order_id','=',$request->order_id)->get(['products.*', 'productcarts.id as cart_id', 'productcarts.user_id', 'productcarts.product_id', 'productcarts.qty as cart_qty', 'productcarts.size as cart_size', 'productcarts.amount', 'productcarts.color as cart_color', 'productcarts.cart_keys as cart_keys', 'productcarts.cart_values as cart_values', 'productcarts.price as cart_price', 'productcarts.shipping_cost as product_shipping_cost']);
        $success['product'] = Productcart::join('products', 'products.id', '=', 'productcarts.product_id')->where('productcarts.is_active','=', '0')->where('productcarts.order_id','=',$request->order_id)->where('productcarts.product_id','=',$request->product_id)->first(['products.*', 'productcarts.id as cart_id', 'productcarts.user_id', 'productcarts.product_id', 'productcarts.qty as cart_qty', 'productcarts.size as cart_size', 'productcarts.amount', 'productcarts.color as cart_color', 'productcarts.cart_keys as cart_keys', 'productcarts.cart_values as cart_values', 'productcarts.price as cart_price', 'productcarts.shipping_cost as product_shipping_cost']);
        $success['order_tracking_data']  = OrderTrack::where('order_id', $request->order_id)->groupBy('title')->get();
        return $this->sendResponse($success, 'Order item detail listed successfully.');   
    }
    ...
    /**
    * Get tracking status detail  api
    *
    * @return \Illuminate\Http\Response
    */
    public function getTrackingStatus(){
        $success['tracking_status']  = array('0'=>'pending', '1'=>'processing', '2'=>'completed', '3'=>'declined', '4'=>'pending', '5'=>'on delivery');
        return $this->sendResponse($success, 'Tracking status listed successfully.');   
    }
}