...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
	...
	public function index(Request $request){
        if(Auth::guard('web')->check()){
            if(Auth::user()->is_vendor== 1 || Auth::user()->is_vendor== 2){
                return redirect()->route('vendor.front.index');
            }
        }
        $this->code_image();
...
// -------------------------------- PRINT SECTION ----------------------------------------
    ...
    public function sendotp($mobile_no=""){
        $user_count =  DB::table('users')->where('phone', $mobile_no)->where('is_vendor', '!=', '0')->count();
        if($user_count > 0){
            return response()->json(array('errors' => [ 0 => 'User already exists.' ]));
        }
        else{
            DB::table('users')->where('phone', $mobile_no)->delete();
            $otp_str  =  rand(10000, 99999);
            $message  =  'Your otp is '.$otp_str;

            $url  = "http://login.yourbulksms.com/api/sendhttp.php?authkey=17856AlUIma1509Q5fe053dcP15&mobiles=".$mobile_no."&message=".urlencode($message)."&sender=RATCRT&route=4&country=91";
            $curl = curl_init();
            ...
            $err = curl_error($curl);
            curl_close($curl); 
            $otp_input = array('mobile_no'=>$mobile_no, 'otp_str'=>$otp_str);
    ...
    public function verifyotp($mobile_no="", $otp=""){
        $get_opt_count =  DB::table('otps')->where('mobile_no', $mobile_no)->where('otp_str', $otp)->orderBy('id', 'desc')->count();
        if($get_opt_count==0){
