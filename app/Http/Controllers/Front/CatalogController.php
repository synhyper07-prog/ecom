...
//        $product->update();
    ...
    // ------------------ Rating SECTION --------------------
        ...
        public function tag(Request $request, $slug){
          ...
          $prods    =  Product::when($slug, function ($query, $slug) {
        ...
        public function ratingWiseProduct(Request $request, $rating){
            if (Session::has('currency')) {
                $curr = Currency::find(Session::get('currency'));
            }
            else{
            ...
            if(!empty($product_id_array)){
              ...
              $product_id_array = array('0');
            }
            $prods       =  Product::when($product_id_array, function ($query, $product_id_array) {
        ...
        public function validate_postal_code($pin_code, $product_id){
            $product = Product::where('id', $product_id)->first();
            $vendor  = User::where('id', $product->user_id)->first('pincode'); 
            $curl    = curl_init();
            curl_setopt_array($curl, array(
                  ...
                  CURLOPT_POSTFIELDS      => '{"data":{"from_pincode":"'.$vendor->pincode.'","to_pincode":"'.$pin_code.'","shipping_length_cms":"'.$product->length.'","shipping_width_cms":"'.$product->width.'","shipping_height_cms":"'.$product->height.'","shipping_weight_kg":"'.$product->weight.'","order_type":"forward","payment_method":"cod","product_mrp":"'.$product->price.'","access_token":"50d95e0f7438515d7d40da216fe1b8f4","secret_key":"cf7cafded40dc92f3ad19dda487626af"}}',
