...
class Product extends Model{
    ...
    public static function showTagsCategoryWise($category) {
        $tags = null;
        $tagz = '';
        $name = Product::where('category_id','=',$category)->pluck('tags')->toArray();
        foreach($name as $nm){
            if(!empty($nm)){
                foreach($nm as $n){
                    $tagz .= $n.',';
    ...
    public static function getSize($slug) {
        $final_array = array();
        $category_id = DB::table('categories')->where('slug', $slug)->first(['id']);
        if(!empty($category_id)){
            $size = Product::where('category_id','=',$category_id->id)->where('size', '!=', '')->pluck('size')->toArray();
            if(!empty($size)){
                foreach ($size as $key => $value) {
                    foreach ($value as $key => $size_val) {
                       array_push($final_array, $size_val);
                    }
                }
            }
        }
    ...
    public static function getSizeByCategoryId($id) {
        $final_array = array();
        $category_id = DB::table('categories')->where('id', $id)->first(['id']);
        if(!empty($category_id)){
            $size = Product::where('category_id','=',$category_id->id)->where('size', '!=', '')->pluck('size')->toArray();
            if(!empty($size)){
                foreach ($size as $key => $value) {
        ...
        $sizes = array_unique($final_array);
        $size_array = array();
        foreach ($sizes as $key => $value) {
            if(!empty($value)){
              array_push($size_array, $value);
            }
        }
        return $size_array;
    }
    ...
    static function getDiscountedProductList(){
        $product_array = array();
        $data = Product::with('user')->where('is_discount','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $stars = Rating::where('product_id',$value->id)->avg('rating');
                $ratings = number_format((float)$stars, 1, '.', '')*20;
                $value->rating =  $ratings;
                array_push($product_array, $value);
            }
        }
        return $product_array;
    }

    static function topRatedProductList(){
        $product_array = array();
        $data = Product::with('user')->where('top','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $stars = Rating::where('product_id',$value->id)->avg('rating');
                $ratings = number_format((float)$stars, 1, '.', '')*20;
    ...
    static function bestProductList(){
        $product_array = array();
        $data = Product::with('user')->where('best','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $stars = Rating::where('product_id',$value->id)->avg('rating');
                $ratings = number_format((float)$stars, 1, '.', '')*20;
    ...
    static function latestProductList(){
        $product_array = array();
        $data = Product::with('user')->where('status','=',1)->orderBy('id','desc')->take(8)->get();
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $stars = Rating::where('product_id',$value->id)->avg('rating');
                $ratings = number_format((float)$stars, 1, '.', '')*20;
    ...
    static function trendingProductList(){
        $product_array = array();
        $data = Product::with('user')->where('trending','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $stars = Rating::where('product_id',$value->id)->avg('rating');
                $ratings = number_format((float)$stars, 1, '.', '')*20;
    ...
    static function saleProductList(){
        $product_array = array();
        $data = Product::with('user')->where('sale','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $stars = Rating::where('product_id',$value->id)->avg('rating');
                $ratings = number_format((float)$stars, 1, '.', '')*20;
