...
class ApiController extends BaseController{ 
    ...
    public function sendOtp(Request $request){
        ...
        $message  =   'RatCart: Your code is '.$otp_str.' xmnlabCin7n';
    ...
    public function getProfileDetail(){
        ...
        $success['profile_picture_base_url']  = 'https://ratcart.com/assets/images/users/'.Auth::guard('api')->user()->profile_picture;
    ...
    public function getDashboardDetail(){
        ...
        $success['data_list']  = array(
                         '0'=>array('name'=>'discount_products', 'type'=>'grid', 'bg_color'=>'#e1f5fe', 'bg_image'=>'', 'count'=>'4', 'row_count'=>'2', 'product_list'=>Product::with('user')->where('is_discount','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get()), 
                         '1'=>array('name'=>'top_products', 'type'=>'grid', 'bg_color'=>'#e1f5fe', 'bg_image'=>'', 'count'=>'4',  'row_count'=>'2', 'product_list'=>Product::with('user')->where('top','=',1)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(8)->get()), 
                         '2'=>array('name'=>'best_products', 'type'=>'slider', 'bg_color'=>'#e1f5fe', 'bg_image'=>'', 'count'=>'0', 'row_count'=>'0', 'product_list'=>Product::with('user')->where('best','=',1)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(6)->get()),
                         '3'=>array('name'=>'latest_products', 'type'=>'grid', 'bg_color'=>'#e1f5fe',  'bg_image'=>'', 'count'=>'3', 'row_count'=>'3' ,'product_list'=>Product::with('user')->where('status','=',1)->orderBy('id','desc')->take(9)->get()),
                         '4'=>array('name'=>'trending_products', 'type'=>'grid', 'bg_color'=>'#e1f5fe', 'bg_image'=>'', 'count'=>'4', 'row_count'=>'2' ,'product_list'=>Product::with('user')->where('trending','=',1)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(9)->get()),
                         '5'=>array('name'=>'sale_products', 'type'=>'grid', 'bg_color'=>'#e1f5fe', 'bg_image'=>'', 'count'=>'4', 'row_count'=>'2', 'product_list'=>Product::with('user')->where('sale','=',1)->where('status','=',1)->select($selectable)->orderBy('id','desc')->take(9)->get()));
        return $this->sendResponse($success, 'Home product listed successfully.');   
    }
    ...
    public function checkout(Request $request){
        ...
        $order_info['cart']               = utf8_encode(bzcompress(serialize($cart), 9));
        ...
        if($request->method=='Cash On Delivery'){
            $message = "You have created successfully order on Ratcart. your order no is ".$order_detail->order_number;
    ...
    public function razorCallback(Request $request){
        ...
        $message = "You have successfully create order on Ratcart. your order no is ".$request['receipt'];
        $this->sendPushNotification($message, Auth::user()->fire_base_token);
    ...
    public function getFilterAttributes(Request $request){
        ...
        $final_array = array();
        $filter_attributes_detail = Category::where('id','=',$request['category_id'])->first(['filter_attributes']);
        if(!empty($filter_attributes_detail->filter_attributes)){
            ...
            foreach ($filter_attributes_array as $key => $value) {
                if($value['filter_attribute'] == 'tag'){
                  $data = array("attribute"=>$value['filter_attribute'], "detail"=>Product::showTagsCategoryWise($request['category_id']));
                }
                else if($value['filter_attribute'] == 'size'){
                  $data = array("attribute"=>$value['filter_attribute'], "detail"=>Product::getSizeByCategoryId($request['category_id']));
                }
                else if($value['filter_attribute'] == 'price'){
                  $data = array("attribute"=>$value['filter_attribute'], "detail"=>array('min_price'=>'100', 'max_price'=>'100000'));
                }
                else{
                  $data = array("attribute"=>$value['filter_attribute'], "detail"=>"");
                }
                array_push($final_array, $data);
            }
        }    
        return $final_array;
        $success['attribute_list'] = $final_array;
        return $this->sendResponse($success, 'Listed successfully');     
    }
    ...
    public function downloadOrderInvoice(Request $request){
      $validator = Validator::make($request->all(), [
        ...
        $success['file_path'] = "http://ratcart.com/invoice_pdf/orderrUSWv5MbLz.pdf";
        return $this->sendResponse($success, 'Listed successfully');     
    }
