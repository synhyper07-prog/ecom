                                ...
                                <tr>
                                    ...
                                    @foreach($cart as $key => $product)
                                        @php  $product_array = json_decode($product->items); @endphp
                                            <tr>
                                            <td><input type="hidden" value="{{$key}}">{{ $product->id }}</td>
                                            <td>

                                                @if($product['item']['id'] != 0)
                                                   @php $user = App\Models\User::find($product['item']['id']); @endphp
                                                @if(isset($user))
                                                    <a target="_blank" href="{{route('admin-vendor-show',$user->id)}}">{{$user->shop_name}}</a>
                                                    @else {{ __('Vendor Removed') }} @endif
                                                    @else 
                                            ...
                                            <td>{{ $product->name }}</td>
                                            <td>{{$order->currency_sign}}{{ round($product->price * $order->currency_value , 2) }}</td></tr>
                                    @endforeach
