                            ...
                            @foreach($orders as $orderr) 
                                ...
                                @foreach($orderr as $order)
                                  ...
                                  <tr>
                                    ...
                                    <td>{{$order->qty}}</td>
                                    <td>{{$order->order->currency_sign}}{{$order->price}}</td>
                                        ...
                                        <div class="action-list">
                                          <a href="{{route('vendor-order-show',$order->order->order_number)}}" class="btn btn-primary product-btn"><i class="fa fa-eye"></i> {{ $langg->lang539 }}</a>
                                              <select class="vendor-btn {{ $order->status }}">
                                                ...
                                                <option value="{{ route('vendor-order-status',['id1' => $order->order->order_number, 'status' => 'on delivery']) }}" {{  $order->status == "on delivery" ? 'selected' : ''  }}>Out for delivery</option>
                                              </select>
                                        </div>
                                      </td>
                                    </tr>
...
<script type="text/javascript">
  ...
  var table = $('#geniustable').DataTable({ ordering: false });                                                         
</script>
{{-- DATA TABLE --}}
@endsection   